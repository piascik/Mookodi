...
/**
 * Routine to do a series of exposures. These start at start_time, and are of length exposure_length_ms.
 * They are done until total_duration_ms time has elapsed. They are done using Andor Run to Abort mode.
 * The data is held in the Andor buffer and retrieved asynchronously to the internal Andor exposure loop.
 * @param start_time A timespec structure holding the start time of the first exposure, or 0 if the exposure is
 *        to be started immediately.
 * @param open_shutter A boolean, if TRUE open the shutter.
 * @param exposure_length_ms The length of one exposure, in milliseconds.
 * @param total_duration_ms The length of time to continue to take exposures for, in milliseconds.
 * @param save_function A function pointer, this is called for each exposure frame read out to enable the
 *        caller to save the data with appropriate headers. The function takes the buffer and it's length as 
 *        parameters, and date_obs (the time that frame was started).
 ...
 * @see CCD_GENERAL_ONE_MILLISECOND_MICROSECOND
 ...
 * @see CCD_Setup_Allocate_Image_Buffer
 * @see CCD_TEMPERATURE_STATUS
 * @see CCD_Temperature_Status_To_String
 * @see CCD_Temperature_Get
 */
int CCD_Exposure_Mult_Runat(struct timespec start_time,int open_shutter,int exposure_length_ms,int total_duration_ms,
						 ...
						 int exposure_index, int andor_index, long accumulation,long series))
{
	...
	float readout_time_ms,readout_overhead_ms,exposure_length_s,andor_exposure_length_s;
	float accumulate,kinetic;
	unsigned int andor_retval;
	int exposure_status,acquisition_counter,done,new_image_count,i,start_time_offset_ms,current_time_offset_ms;
	int accumulation,series,first_new_image_index,last_new_image_index;
	int retval,elapsed_time;
	int detector_ncols,detector_nrows,last_last_new_image_index;

	Exposure_Error_Number = 0;
	/* check arguments */
	if(!CCD_GENERAL_IS_BOOLEAN(open_shutter))
	{
		Exposure_Error_Number = 18;
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat:Illegal value:open_shutter = %d.",
			open_shutter);
		return FALSE;
	}
	if(save_function == NULL)
	{
		...
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat:save_function was NULL.");
		return FALSE;
	}
#if LOGGING > 1
	CCD_General_Log("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_INTERMEDIATE,NULL,
			"CCD_Exposure_Mult_Runat started.");
#endif
	/* reset abort */
	Exposure_Data.Abort = FALSE;
#if LOGGING > 5
	CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_INTERMEDIATE,NULL,
	"CCD_Exposure_Mult_Runat(open_shutter=%d,start_time=%ld,exposure_length=%d ms,total_duration=%d ms).",
			       open_shutter,start_time.tv_sec,exposure_length_ms,total_duration_ms);
#endif
	/* set exposure length */
	Exposure_Data.Exposure_Length = exposure_length_ms;
	exposure_length_s = ((float)exposure_length_ms)/1000.0f;
	/* set other parameters used for status reporting */
	Exposure_Data.Exposure_Count = 0;
	Exposure_Data.Exposure_Index = 0;
	Exposure_Data.Accumulation = -1;
	Exposure_Data.Series = -1;
#if LOGGING > 5
	CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_INTERMEDIATE,NULL,
			"SetExposureTime(%.2f).",exposure_length_s);
#endif
	andor_retval = SetExposureTime(exposure_length_s);/* in seconds */
	if(andor_retval != DRV_SUCCESS)
	{
		Exposure_Error_Number = 20;
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat: SetExposureTime(%f) failed %s(%u).",
			exposure_length_s,CCD_General_Andor_ErrorCode_To_String(andor_retval),
			andor_retval);
		return FALSE;
	}
	/* allocate image */
#if LOGGING > 5
	CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_VERBOSE,NULL,
			      "Allocating image buffer.");
#endif
	if(!CCD_Setup_Allocate_Image_Buffer(((void **)&image_data),&image_data_length))
	{
		Exposure_Error_Number = 21;
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat:CCD_Setup_Allocate_Image_Buffer failed.");
		return FALSE;
	}
#if LOGGING > 5
	CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_VERBOSE,NULL,
...
#endif
#if LOGGING > 0
	CCD_General_Log("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_VERBOSE,"ANDOR",
		       "CCD_Exposure_Mult_Runat:SetAcquisitionMode(5):run to abort.");
#endif
	andor_retval = SetAcquisitionMode(5);
	if(andor_retval != DRV_SUCCESS)
	{
		Exposure_Error_Number = 22;
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat:SetAcquisitionMode(5) failed %d(%s).",
			andor_retval,CCD_General_Andor_ErrorCode_To_String(andor_retval));
		Exposure_Data.Exposure_Status = CCD_EXPOSURE_STATUS_NONE;
		if(image_data != NULL)
			free(image_data);
	...
	/* get temperature */
#if LOGGING > 5
	CCD_General_Log("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_VERBOSE,NULL,
		       "Getting current CCD temperature.");
#endif
	/* this sets the cached temperature, which will then be put in FITS headers,
	** as Andor library is always in acquiring mode */
	if(!CCD_Temperature_Get(&current_temperature,&temperature_status))
	{
		Exposure_Error_Number = 25;
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat:CCD_Temperature_Get failed.");
		if(image_data != NULL)
			free(image_data);
		return FALSE;
	}
#if LOGGING > 5
	CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_VERBOSE,NULL,
			      "Current CCD temperature is %.2f C with status %s.",current_temperature,
			      CCD_Temperature_Status_To_String(temperature_status));
#endif
	/* check exposure length is sane */
	andor_retval = GetAcquisitionTimings(&andor_exposure_length_s,&accumulate,&kinetic);
	if(andor_retval != DRV_SUCCESS)
	{
		if(image_data != NULL)
			free(image_data);
		Exposure_Error_Number = 35;
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat: GetAcquisitionTimings() failed %s(%u).",
			CCD_General_Andor_ErrorCode_To_String(andor_retval),andor_retval);
		return FALSE;
	}
#if LOGGING > 5
	CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_VERBOSE,NULL,
			   "GetAcquisitionTimings returned exposure length %.3f s, accumulate %.3f s, kinetic %.3f s.",
			       andor_exposure_length_s,accumulate,kinetic);
#endif
	/* check andor library is not increasing your exposure length */
	if(andor_exposure_length_s > exposure_length_s)
	{
		if(image_data != NULL)
			free(image_data);
		Exposure_Error_Number = 36;
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat: GetAcquisitionTimings() suggests "
			"minimum exposure length %.6f s too long for required exposure length %.6f.",
			andor_exposure_length_s,exposure_length_s);
		return FALSE;
	}
	/* wait for start time */
#if LOGGING > 5
	CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_VERBOSE,NULL,
			      "Waiting for Exposure start time(%ld).",start_time.tv_sec);
#endif
	...
	if(open_shutter)
	{
		...
		CCD_General_Log("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_INTERMEDIATE,NULL,
				"SetShutter(1,1,0,0).");
#endif
		andor_retval = SetShutter(1,1,0,0);
		if(andor_retval != DRV_SUCCESS)
		{
			if(image_data != NULL)
				free(image_data);
			Exposure_Error_Number = 23;
			sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat: SetShutter() failed %s(%u).",
				CCD_General_Andor_ErrorCode_To_String(andor_retval),andor_retval);
			return FALSE;
		}
	}
	...
	if(!CCD_Setup_Get_Buffer_Length(&pixel_count))
	{
		...
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat: CCD_Setup_Get_Buffer_Length failed.");
		return FALSE;
	}
	andor_pixel_count = (at_u32)pixel_count;
	/* get readout time */
	...
	*/
	detector_ncols = CCD_Setup_Get_Detector_Pixel_Count_X();
	detector_nrows = CCD_Setup_Get_Detector_Pixel_Count_Y();
	/* diddly binned
	readout_time_ms = ((binned_nrows*CCD_Setup_Get_VSSpeed())+(binned_nrows*binned_ncols*CCD_Setup_Get_HSSpeed()))/
		CCD_GENERAL_ONE_MILLISECOND_MICROSECOND;
	*/
	/* diddly delete
	if(CCD_Setup_Get_HSSpeed() < 0.001f)
	{
		if(image_data != NULL)
			free(image_data);
		Exposure_Error_Number = 39;
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat: Illegal Horizontal Shift Speed %.3f (%d).",
			CCD_Setup_Get_HSSpeed(),CCD_Setup_Get_HSSpeed());
		return FALSE;
	}
	*/
	readout_time_ms = 10000;
	/*
	((detector_nrows*CCD_Setup_Get_VSSpeed())+
			   ((detector_nrows*detector_ncols)/CCD_Setup_Get_HSSpeed()))/
		           CCD_GENERAL_ONE_MILLISECOND_MICROSECOND;
	*/
	readout_overhead_ms = readout_time_ms;
#if LOGGING > 5
	CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_VERBOSE,NULL,
			      "detector_nrows = %d, detector_ncols = %d, readout time = %.3f ms, "
			       "exposure length = %d ms, readout_overhead = %.3f ms.",detector_nrows,detector_ncols,
			       readout_time_ms,exposure_length_ms,readout_overhead_ms);
#endif
	/* reset exposure start time here */
#ifdef _POSIX_TIMERS
	clock_gettime(CLOCK_REALTIME,&(Exposure_Data.Start_Time));
#else
	gettimeofday(&gtod_current_time,NULL);
...
#if LOGGING > 5
	CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_VERBOSE,NULL,
			      "Starting acquisitions:StartAcquisition.");
#endif
	andor_retval = StartAcquisition();
	if(andor_retval != DRV_SUCCESS)
	{
		Exposure_Error_Number = 26;
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat:StartAcquisition failed %d(%s).",
			andor_retval,CCD_General_Andor_ErrorCode_To_String(andor_retval));
		Exposure_Data.Exposure_Status = CCD_EXPOSURE_STATUS_NONE;
		SetShutter(1,2,0,0); /* close */
		if(image_data != NULL)
			free(image_data);
		return FALSE;
	}
	...
	Exposure_Data.Exposure_Index = 0;
	Exposure_Data.Exposure_Count = 0; /* not a fixed number for mult_runat */
	Exposure_Data.Accumulation = 0;
	Exposure_Data.Series = 0;
	last_last_new_image_index = 0;
	while(done == FALSE)
	{
#if LOGGING > 9
		CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",
				      LOG_VERBOSITY_VERY_VERBOSE,NULL,"Waiting for acquisition.");
#endif
		/* wait for an acquisition to complete */
		andor_retval = WaitForAcquisition();
		if(andor_retval != DRV_SUCCESS)
		{
			Exposure_Error_Number = 27;
			sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat:WaitForAcquisition failed %d(%s).",
				andor_retval,CCD_General_Andor_ErrorCode_To_String(andor_retval));
			Exposure_Data.Exposure_Status = CCD_EXPOSURE_STATUS_NONE;
			Exposure_Data.Exposure_Index = 0;
			Exposure_Data.Accumulation = 0;			
		...
		if(andor_retval != DRV_SUCCESS)
		{
			...
			sprintf(Exposure_Error_String,
				"CCD_Exposure_Mult_Runat:GetAcquisitionProgress failed %d(%s).",
				andor_retval,CCD_General_Andor_ErrorCode_To_String(andor_retval));
			Exposure_Data.Exposure_Status = CCD_EXPOSURE_STATUS_NONE;
			Exposure_Data.Exposure_Index = 0;
			Exposure_Data.Accumulation = 0;
		...
		CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",
				      ...
				      "Acquisition progress:acquisition=%d, series = %d.",
				      Exposure_Data.Accumulation,Exposure_Data.Series);
#endif
		andor_retval = GetNumberNewImages(&first_new_image_index,&last_new_image_index);
		if(andor_retval != DRV_SUCCESS)
		{
			if(andor_retval == DRV_NO_NEW_DATA)
			{
				/* this is not necessarily an error, sometimes WaitForAcquisition misses
				** a few frames and then triggers multiple times whilst acquiring the same frame.
				** We set the first new image index to be greater than the last, 
				** to stop the for loop below being entered, therefore no new frames are saved. */
#if LOGGING > 9
				CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",
						      LOG_VERBOSITY_VERY_VERBOSE,NULL,
		   "GetNumberNewImages returned DRV_NO_NEW_DATA:Faking number of new images so none are saved.");
#endif
				first_new_image_index = last_new_image_index+1;
			}
			else
			{
				Exposure_Error_Number = 29;
				sprintf(Exposure_Error_String,
					"CCD_Exposure_Mult_Runat:GetNumberNewImages failed %d(%s).",
					andor_retval,CCD_General_Andor_ErrorCode_To_String(andor_retval));
				Exposure_Data.Exposure_Status = CCD_EXPOSURE_STATUS_NONE;
				Exposure_Data.Exposure_Index = 0;
				Exposure_Data.Accumulation = 0;
		...
		{
			if(first_new_image_index != (last_last_new_image_index+1))
			{
#if LOGGING > 1
				CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",
						       LOG_VERBOSITY_VERY_VERBOSE,NULL,
						       "Dropped frames warning between %d and %d.",
						       last_last_new_image_index,first_new_image_index);
#endif
			}
		}
		new_image_count = (int)((last_new_image_index-first_new_image_index)+1);
#if LOGGING > 9
		CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",
				      LOG_VERBOSITY_VERY_VERBOSE,NULL,
				      "Number of new images from index:first=%ld, to last=%ld : new images = %d.",
				      first_new_image_index,last_new_image_index,new_image_count);
#endif
		/* retrieve and save all new images */
		for(i = first_new_image_index; i <= last_new_image_index; i++)
		{
#if LOGGING > 9
			CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",
					      LOG_VERBOSITY_VERY_VERBOSE,NULL,
					      "Calling GetOldestImage16(pixel_count=%d) for index %d.",
					       andor_pixel_count,i);
#endif
			andor_retval = GetOldestImage16(image_data,andor_pixel_count);
			if(andor_retval != DRV_SUCCESS)
			{
				Exposure_Error_Number = 30;
				sprintf(Exposure_Error_String,
					"CCD_Exposure_Mult_Runat:GetOldestImage16(%p,%u) failed %d(%s).",
					(void*)image_data,andor_pixel_count,
					andor_retval,CCD_General_Andor_ErrorCode_To_String(andor_retval));
				Exposure_Data.Exposure_Status = CCD_EXPOSURE_STATUS_NONE;
				Exposure_Data.Exposure_Index = 0;
				Exposure_Data.Accumulation = 0;
			...
			/* calculate offset between StartAcquisition and the start of this frame in ms */
			/* diddly really use ReadoutTime here? Or frame transfer time as readout
			** is done whilst next exposure starts. Or kinetic cycle time? as we have to
			** wait for some readout to be completed for very short exposures? */
			/* see also rise:ccd_multrun.c:start_time_correction/ correct_start_time*/
			/* get date timestamp */
#ifdef _POSIX_TIMERS
			clock_gettime(CLOCK_REALTIME,&current_time);
#else
...
#endif
			/*current_time_offset_ms = (((i-first_new_image_index)+1)*exposure_length_ms);*/
			current_time_offset_ms = ((((last_new_image_index-i)+1)*exposure_length_ms));

			date_obs.tv_sec = current_time.tv_sec-(current_time_offset_ms/1000.0f);
			date_obs.tv_nsec = current_time.tv_nsec-
				((current_time_offset_ms%1000)*CCD_GENERAL_ONE_MILLISECOND_NS);
			if(date_obs.tv_nsec < 0)
			{
				date_obs.tv_nsec += CCD_GENERAL_ONE_SECOND_NS;
				date_obs.tv_sec--;
			}
			/* This produces DATE-OBS after the current time which can't be right */
			/*
			start_time_offset_ms = (exposure_length_ms+(readout_overhead_ms))*Exposure_Data.Exposure_Index;
			date_obs.tv_sec = Exposure_Data.Start_Time.tv_sec+(start_time_offset_ms/1000.0f);
			date_obs.tv_nsec = Exposure_Data.Start_Time.tv_nsec+
				((start_time_offset_ms%1000)*CCD_GENERAL_ONE_MILLISECOND_NS);
			if(date_obs.tv_nsec > CCD_GENERAL_ONE_SECOND_NS)
			{
				date_obs.tv_nsec -= CCD_GENERAL_ONE_SECOND_NS;
				date_obs.tv_sec++;
			}
			*/
#if LOGGING > 5
			start_time_offset_ms = (((i-first_new_image_index)+1)*exposure_length_ms);
			CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",
					      LOG_VERBOSITY_INTERMEDIATE,NULL,
					       "start time offset (%d + %.2f) * %d = %d ms.",exposure_length_ms,
					       readout_time_ms,Exposure_Data.Exposure_Index,start_time_offset_ms);
			/*
			CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",
					      LOG_VERBOSITY_INTERMEDIATE,NULL,
					       "start time offset (%d + %.2f) * %d = %d ms.",exposure_length_ms,
					       readout_time_ms,Exposure_Data.Exposure_Index,start_time_offset_ms);
			*/
			CCD_General_Get_Time_String(date_obs,time_string,32);
			CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",
					      LOG_VERBOSITY_INTERMEDIATE,NULL,
					      "DATE-OBS for index %d calculated to be %s",
					       Exposure_Data.Exposure_Index,time_string);
#endif
			retval =  (*save_function)(image_data,image_data_length,date_obs,
						   Exposure_Data.Exposure_Index, i, Exposure_Data.Accumulation,
						   Exposure_Data.Series);
			if(retval == FALSE)
			{
				Exposure_Error_Number = 34;
				sprintf(Exposure_Error_String,
					"CCD_Exposure_Mult_Runat:save function %p failed.",save_function);
				Exposure_Data.Exposure_Status = CCD_EXPOSURE_STATUS_NONE;
				Exposure_Data.Exposure_Index = 0;
				Exposure_Data.Accumulation = 0;
				Exposure_Data.Series = 0;
		...
		/* save andor index of last image saved to last_last_new_image_index */
		last_last_new_image_index = last_new_image_index;
		/* check whether to terminate exposure taking 
		** check if we are in elapsed time mode, and sufficient time has elapsed. */
#ifdef _POSIX_TIMERS
		clock_gettime(CLOCK_REALTIME,&current_time);
#else
		gettimeofday(&gtod_current_time,NULL);
		...
		current_time.tv_nsec = gtod_current_time.tv_usec*CCD_GENERAL_ONE_MICROSECOND_NS;
#endif
		elapsed_time = (int)(fdifftime(current_time,Exposure_Data.Start_Time)*
		...
		if(elapsed_time > total_duration_ms)
		{
			...
			CCD_General_Log_Format("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",
					      LOG_VERBOSITY_INTERMEDIATE,NULL,
					   "Acquisition stopping:elapsed time %d > total duration %d after %d frames.",
					      elapsed_time,total_duration_ms,Exposure_Data.Exposure_Index);
#endif
		}
		if(Exposure_Data.Abort == TRUE)
		{
			done = TRUE;
#if LOGGING > 5
			CCD_General_Log("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",
				       LOG_VERBOSITY_INTERMEDIATE,NULL,"Acquisition stopping:Aborting.");
#endif
		}
	}/* end while */
	/* stop the Andor camera taking images */
#if LOGGING > 5
	CCD_General_Log("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_INTERMEDIATE,NULL,
	...
	if(andor_retval != DRV_SUCCESS)
	{
		...
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat:AbortAcquisition failed %d(%s).",
			andor_retval,CCD_General_Andor_ErrorCode_To_String(andor_retval));
		Exposure_Data.Exposure_Status = CCD_EXPOSURE_STATUS_NONE;
		Exposure_Data.Exposure_Index = 0;
		Exposure_Data.Accumulation = 0;
	...
	CCD_General_Log("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_VERBOSE,NULL,
		       "Closing shutter.");
#endif
	andor_retval = SetShutter(1,2,0,0); /* close */
	if(andor_retval != DRV_SUCCESS)
	{
		Exposure_Error_Number = 32;
		sprintf(Exposure_Error_String,"CCD_Exposure_Mult_Runat:SetShutter failed %d(%s).",
			andor_retval,CCD_General_Andor_ErrorCode_To_String(andor_retval));
		Exposure_Data.Exposure_Status = CCD_EXPOSURE_STATUS_NONE;
		Exposure_Data.Exposure_Index = 0;
		Exposure_Data.Accumulation = 0;
...
#if LOGGING > 0
	CCD_General_Log("ccd","ccd_exposure.c","CCD_Exposure_Mult_Runat",LOG_VERBOSITY_INTERMEDIATE,NULL,
			"CCD_Exposure_Mult_Runat() returned TRUE.");
#endif
	return TRUE;
}
